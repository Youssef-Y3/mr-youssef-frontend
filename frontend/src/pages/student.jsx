...
function StudentLecturesPage() {
  ...
  React.useEffect(() => {
    studentApi.lectures().then(r => setLectures(Array.isArray(r) ? r : (r?.lectures || []))).catch(e => setError(e.message));
  }, []);

  return (
    <StudentLayout page="lectures">
      <FadeIn>
        <h1 className="font-display font-black text-3xl text-azhar-800 mb-2">المحاضرات</h1>
        <p className="text-ink/60 mb-6">محاضرات مستقلة بتترفع طول السنة</p>
      </FadeIn>
      {error && <Card className="p-4 bg-red-50 border-red-200 text-red-700 mb-4">{error}</Card>}
      {!lectures ? (
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">{Array.from({ length: 6 }).map((_, i) => <Skeleton key={i} className="h-40 rounded-2xl" />)}</div>
      ) : lectures.length ? (
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {lectures.map(l => (
            l.locked ? (
              <div key={l.id} className="bg-white rounded-2xl shadow-soft p-5 opacity-60 cursor-not-allowed">
                <div className="w-10 h-10 rounded-xl bg-slate-100 text-slate-500 flex items-center justify-center mb-3">🔒</div>
                <div className="font-display font-bold text-ink/60">{l.title}</div>
                <div className="text-xs text-ink/40 mt-1">اشترك عشان تفتح المحاضرة دي</div>
              </div>
            ) : (
              <a key={l.id} href={`#/lecture/${l.id}`} className="bg-white rounded-2xl shadow-soft p-5 card-hover block">
                <div className="w-10 h-10 rounded-xl bg-gold-100 text-gold-700 flex items-center justify-center mb-3"><Icon.Play className="w-5 h-5" /></div>
                <div className="font-display font-bold text-azhar-800">{l.title}</div>
                {l.is_free ? <span className="text-xs bg-emerald-100 text-emerald-800 px-2 py-0.5 rounded-full font-bold mt-2 inline-block">مجانية</span> : null}
              </a>
            )
          ))}
        </div>
      ) : (
        <Card className="p-10 text-center text-ink/60">لا توجد محاضرات متاحة حالياً</Card>
      )}
    </StudentLayout>
  );
}
    ...
    <StudentLayout page="lectures">
      ...
      {!data ? (
        ...
        <Card className="p-14 text-center">
          <div className="w-16 h-16 mx-auto rounded-2xl bg-gold-100 text-gold-600 flex items-center justify-center mb-4"><Icon.CreditCard className="w-8 h-8" /></div>
          <h3 className="font-display font-black text-2xl text-azhar-800 mb-2">المحاضرة دي للمشتركين بس</h3>
          <p className="text-ink/70 mb-6">فعّل اشتراكك عشان تقدر تشوفها</p>
          <Button variant="primary" onClick={() => nav("/plans-me")}>اشترك الآن</Button>
        </Card>
      ) : (
        <div>
          ...
          <div className="relative bg-black rounded-3xl overflow-hidden shadow-lift aspect-video">
            {data.video_url ? (
              ...
              <div className="aspect-video flex items-center justify-center text-white/60">الفيديو غير متاح حالياً</div>
            )}
          </div>
        </div>
      )}
...
Object.assign(window, {
  StudentLayout, StudentDashboard, StatCard, StudentCoursesPage, CourseWithProgress, CourseDetailPage, UnitAccordion, LivePage,
  StudentLecturesPage, LectureDetailPage,
});
